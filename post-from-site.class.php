...
class PostFromSite {
	...
	public function get_form(){
		...
		<script type="text/javascript">		
			...
			function desabilita() {
						...
						if (document.getElementById("pfs-termosdiv-input").checked == true) 
		...
		//$pageid = get_page_by_title('Termos de serviço');
		$pageid = 86;
		...
		if (current_user_can('publish_posts') || $options['allow_anon']){
			...
			$out .= "<input type='hidden' name='MAX_FILE_SIZE' value='" .apply_filters('pfs_maxfilesize',52428800). "' />\n";					
			$out .= apply_filters( 'pfs_form_start', '', $idtext );
			$out .= "<label for='pfs_title'>". __('T&iacute;tulo:','pfs_domain'). "</label> <input name='title' id='pfs_title' value='' type='text' class='input-text' />\n";
			if (!current_user_can('publish_posts') && $options['allow_anon']){ //if not logged in/able to publish posts, and anon posting allowed, show name/email
				$out .= "<label for='pfs_name'>".__('Nome:','pfs_domain')."</label> <input name='name' id='pfs_name' class='input-text' value='' type='text' />";
				$out .= "<label for='pfs_email'>".__('Email:','pfs_domain')."</label> <input name='email' id='pfs_email' class='input-text' value='' type='email' />\n";
			}
			$out .= "<label for='postcontent'>". __('Descri&ccedil;&atilde;o:','pfs_domain'). "</label><textarea id='postcontent' name='postcontent' rows='12' cols='50'></textarea>\n";
			...
			//if ($options['allow_image']) {
				$out .= "<label for='pfs_imgdiv$idtext'>". __('Arquivo:','pfs_domain') ."</label>";
				...
				$out .= "<div id='pfs-imgdiv'><input id ='pfs-imgdiv-input' type='file' class='multi' name='file-upload[]'/></div>\n";				
			//}
			$out .= "<label for='terms-text'>". __('Termos de Servi&ccedil;o:','pfs_domain'). "</label><textarea id='terms-text' name='terms-text' rows='12' cols='50' readonly='readonly'> \n". $pagedata->post_content."</textarea>\n";
			$out .= "<div id='pfs_termosdiv'><input type='checkbox' name='pfs-termosdiv-input' id='pfs-termosdiv-input' onClick='desabilita()' /></div>";		
			$out .= "Li e concordo com os Termos de Servi&ccedil;o";
			...
			$out .= "<span id='erro-termos' name='erro-termos'></span> \n";
			$out .= apply_filters( 'pfs_form_end', '', $idtext );
			$out .= "</form>\n<div class='clear'></div>\n";
			$out .= apply_filters( 'pfs_after_form', '', $idtext );
		} else {
			$out .= apply_filters( 'pfs_alert_login', "<p>Voce deve estar logado para criar um post.</p>" );
		}
		$out .= "</div>\n\n";
		return $out;
	}	
	...
	public function get_taxonomy_list( $taxonomy ){
		...
		$out = apply_filters( 'pfs_taxonomy_label', "<label for='terms_$taxonomy'>Lista de Tags:</label>", $taxonomy );
